...
def main():
    ...
    # 与 benchmark 对齐，用模型加载前的 ambient idle（~80W）
    if idle_before > 0:
        P_bl = idle_before
    else:
        P_bl = 0.0
    ...
    for name, s, e in phases:
        ...
        # ---- Category breakdown via kernel profiling ratios ----
        ...
        if _r_sum > 0:
            cat_results = {
                "compute_bound": e_j * _r_c / _r_sum,
                "memory_bound": e_j * _r_m / _r_sum,
                "data_movement": e_j * _r_d / _r_sum,
                "communication": e_j * _r_co / _r_sum,
            }
