...
def run_cmd(cmd: list, step_label: str) -> bool:
    """运行命令并实时输出，返回 True 表示成功。"""
    print(f"\n{'=' * 60}")
    print(f"  [{step_label}]")
...
def main():
    ...
    for model_name in models:
        ...
        ok = run_cmd(run_args, f"{model_name}: run.py")
        if not ok:
